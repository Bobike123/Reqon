...
begin
  ...
  insert into checks (label, who, stmt, kind, expected) values
    ...
    ('developer     adds an entry',    dev, format('insert into finance_entries (season_id, kind, description, amount_cents) values (%L, ''income'', ''dev entry'', 100)', season), 'write', 'ALLOWED'),
    ...
    ('developer     edits an entry',   dev, 'update finance_entries set amount_cents = 1 where description = ''dev entry''', 'write', 'ALLOWED'),
    ...
    ('developer     deletes an entry', dev, 'delete from finance_entries where description = ''dev entry''', 'write', 'ALLOWED'),
